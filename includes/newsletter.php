...
class websten_pointers {
	...
	function admin_enqueue_scripts () {
		...
		$do_tour = !in_array ('websten_subscribe_pointer', $dismissed);
		if (true) {
	...
	function admin_print_footer_scripts () {
        ...
        if (!array_key_exists($tab, $tour)) {
			...
			$id = '.settings_page_web-stories-enhancer';  // Define ID used on page html element where we want to display pointer

			$content = '<h3>' . sprintf (esc_html__('Thank You for using Web stories enhancer plugin!', 'web_stories_enhancer')) . '</h3>';
			$content .= '<p>' . esc_html__('Do you want the latest update on', 'web_stories_enhancer') . '<b>' . esc_html__(' Web stories enhancer', 'web_stories_enhancer') . '</b>' . esc_html__('before others and some best resources on Web stories in a single email? - Free just for users of this plugin', 'web_stories_enhancer').'</p>';
            $content .= '
		...
		if ($show_pointer) {
			//var_dump($id);die;
			$this->websten_pointer_script ($id, $options, esc_html__('No Thanks', 'web_stories_enhancer'), $button2, $function);
		}
	}
	...
	function websten_pointer_script ($id, $options, $button1, $button2=false, $function='') {
		?>
		<script type="text/javascript">
			(function ($) {
				...
				wp_pointers_tour_opts = $.extend (wp_pointers_tour_opts, {
					...
					close: function () {
						$.post (ajaxurl, {
							pointer: 'websten_subscribe_pointer',
...
$websten_pointers = new websten_pointers();
?>